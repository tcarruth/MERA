...
# ============= Risk Assessment ==================
  ...
  Figs[[2]]<-function(MSEobj,MSEobj_reb,options=list()) BMSYproj(MSEobj,MSEobj_reb,options,maxcol=4)
...
# ============= Planning =========================
  ...
  Tabs[[1]]<-function(MSEobj,MSEobj_reb,options=list(res=1),rnd=1){
    ...
    Tab1<-as.data.frame(cbind(MSEobj@MPs,LRP),stringsAsFactors = F)
    for(i in 2:ncol(Tab1))Tab1[,i]<-as.numeric(Tab1[,i])
    colnams<-c("MP",ind+Current_Year)
    names(Tab1)<-colnams
    Bdeps<-MSEobj@OM$D/MSEobj@OM$SSBMSY_SSB0 #MSEobj_reb@B_BMSY[,1,1]#
  ...
  Tabs[[2]]<-function(MSEobj,MSEobj_reb,options=list(res=1),rnd=1){
    ...
    Tab2<-as.data.frame(cbind(MSEobj@MPs,TRP),stringsAsFactors = F)
    for(i in 2:ncol(Tab2))Tab2[,i]<-as.numeric(Tab2[,i])
    colnams<-c("MP",ind+Current_Year)
    names(Tab2)<-colnams
    Bdeps<-MSEobj@OM$D/MSEobj@OM$SSBMSY_SSB0 #MSEobj_reb@B_BMSY[,1,1]#
