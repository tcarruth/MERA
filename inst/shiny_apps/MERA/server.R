...
#############################################################################################################################################################################
  ...
  output$Build_App <- downloadHandler(
    ...
    content = function(file) {
      ...
      params <- list(test = input$Name,
                     ...
                     set_type=paste0("Application of an MP"," (MERA version ",Version,")"),
  ...
  observeEvent(input$All_Ah,
     if(input$All_Ah  == 0 | input$All_Ah%%2 == 0){
       updateCheckboxGroupInput(session,"Ah",choices=Ah_list,selected=Ah_list)
     }else{
       updateCheckboxGroupInput(session,"Ah",choices=Ah_list)
     }
  )
  observeEvent(input$All_Vh,
     if(input$All_Vh  == 0 | input$All_Vh%%2 == 0){
       updateCheckboxGroupInput(session,"Vh",choices=Vh_list,selected=Vh_list)
     }else{
       updateCheckboxGroupInput(session,"Vh",choices=Vh_list)
     }
  )
