...
redoEval<-function(fease=F){
  ...
  withProgress(message = "Calculating Evaluation results", value = 0, {
    ...
    test<-function(MSEobj,planning_phase=50,rnd=1,res=5){
    ...
    # Planning slot 1 are the table functions
    # Planning slot 2 are the figure functions
    output$P_Tab_1_title=renderText(Skin$Planning$Tab_title[[1]])
    output$P_Tab_1_text=renderText(Skin$Planning$Tab_text[[1]])
    output$P_Tab_1 <- DT::renderDataTable(Skin$Planning[[1]][[1]](MSEobj),options = list(dom = 't')) 
    
    output$P_Tab_2_title=renderText(Skin$Planning$Tab_title[[2]])
    output$P_Tab_2_text=renderText(Skin$Planning$Tab_text[[2]])
    output$P_Tab_2 <- DT::renderDataTable(Skin$Planning[[1]][[2]](MSEobj),options = list(dom = 't')) 
    
    output$P_Tab_3_title=renderText(Skin$Planning$Tab_title[[3]])
    output$P_Tab_3_text=renderText(Skin$Planning$Tab_text[[3]])
    output$P_Tab_3 <- DT::renderDataTable(Skin$Planning[[1]][[3]](MSEobj),options = list(dom = 't')) 
    
    output$P_Fig_1_title=renderText(Skin$Planning$Fig_title[[1]])
    output$P_Fig_1_text=renderText(Skin$Planning$Fig_text[[1]])
    output$P_Fig_1<-renderPlot(Skin$Planning[[2]][[1]](MSEobj), height =ceiling(nMPs/6)*320 , width = 1300)
    
    output$P_Fig_2_title=renderText(Skin$Planning$Fig_title[[2]])
    output$P_Fig_2_text=renderText(Skin$Planning$Fig_text[[2]])
    output$P_Fig_2<-renderPlot(Skin$Planning[[2]][[2]](MSEobj_reb), height =ceiling(nMPs/6)*320 , width = 1300)
